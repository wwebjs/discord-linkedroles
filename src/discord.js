...
/**
 * Given a user based access token, fetch information about the current user's GitHub connection.
 */
export async function getUsersGithubConnection(tokens) {
  ...
  if (response.ok) {
    const data = await response.json();
    const connection = data.find((c) => c.type === 'github');
    return connection;
  } else {
    throw new Error(`Error fetching user data: [${response.status}] ${response.statusText}`);
  }
